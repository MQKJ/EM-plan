<template> 
  <div class="contentBx"> 
      ...
      <div class="sign-bx">
        ...
        <div class="title">
          <ul class="">
            <li class="left s-l-i" style="width:70%">
              <div class="left s-title">
                <p class="">
                  现场采集人脸照片(自拍正脸照)
                </p>
            ...
            <li class="left s-r-i" style="width:30%">
              <div class="left s-line">
                <div class="">
                  <img src="../assets/images/y_sign_line.png" alt="" class="">
                </div>
  ...
  methods: {
    ...
    cropp () {
        ...
        img.onload = function () {
          ...
          var ctx;
          var img64;
          var sW = $('#header').width(); // 获取显示图片的宽度(必须值)
          var sH = $('#header').height(); // 获取显示图片的高度(必须值)
          // 与裁切图片等比例压缩就需要有宽高比例
          var rate = sW / sH; // 宽高比例
          if (rate <= 0.15) { // 由于用户裁切出的图片宽高比不定，以宽高比0.15分界，如果小于0.15，那么绘制出的图片会抬高，这样会大于200k；
            rate = 0.15;
            canvas = document.createElement('canvas');
            canvas.width = 200;
            canvas.height = 200 / rate; // 设置创建的canvas的宽度200高度按等比例计算
    ...
    postImg () {
      ...
      var param = {
          ...
          isLiveCheck: true // true: 需要做活体检测 false: 不做活体检测
        };
